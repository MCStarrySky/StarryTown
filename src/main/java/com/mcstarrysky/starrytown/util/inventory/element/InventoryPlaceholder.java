...
public class InventoryPlaceholder implements InventoryElement {
    ...
    public void click(final PInventory<?> holder, final InventoryClickEvent event) {
        ...
        LocaleUtil.debug("InventoryPlaceholder {0} 已被点击:", getBase().getName());
        LocaleUtil.debug("槽位内物品: {0}", slotItem);
        LocaleUtil.debug("指针上物品: {0}", cursorItem);
        LocaleUtil.debug("点击操作: {0}", event.getAction());
        LocaleUtil.debug("当前已放置物品集: {0}", this.placedMap);
        ...
        LocaleUtil.debug("事件处理结果: {0}", (Object) event.isCancelled());
