package com.mcstarrysky.starrytown.command;
...
import com.mcstarrysky.starrytown.inventory.TownInfoInventory;
import com.mcstarrysky.starrytown.item.Items;
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args.length == 0) {
            ...
            switch (args[0].toLowerCase()) {
                    ...
                    if (sender instanceof Player) {
                        ...
                        if (Cache.REMOVE_CACHE.containsKey(p.getUniqueId())) {
                            ...
                            if (town.isOwner(p.getUniqueId())) {
                                ...
                                LocaleUtil.send(p, "已经解散聚落 <green>" + town.getName() + "<white>!");
                            } else {
                ...
                case "get" -> {
                    if (args.length != 2) {
                        LocaleUtil.send(sender, "参数长度有误!");
                    } else {
                        final String name = args[1];
                        final Player p1 = Bukkit.getPlayerExact(name);
                        if (p1 == null) {
                            LocaleUtil.send(sender, "玩家不在线或是不存在!");
                        } else {
                            p1.getInventory().addItem(Items.TOWN_CREATE_PAPER).forEach((integer, itemStack) -> p1.getWorld().dropItem(p1.getLocation(), itemStack));
                            LocaleUtil.send(p1, "你已成功获取一张聚落创建凭证! 手持输入 <green>/starrytown create <名字> <white>来创建!");
                            LocaleUtil.send(sender, "操作成功! 该玩家已收到一张聚落创建凭证.");
                        }
                    }
                }
                    /*
                ...
                case "economy":
                    ...
                    break;
                     */
                default -> LocaleUtil.send(sender, "未知命令, 请检查你输入的命令是否正确.");
            }
        }
        return true;
    }
